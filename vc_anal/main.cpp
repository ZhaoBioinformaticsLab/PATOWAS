...
double fminfn(int n, double *p, void *ex, int Matrix_Size, double *kk_matrix, double *genotype_vector, double *phenotype_vector);
void fmingr(int n, double *p, double *df, void *ex, int Matrix_Size, double *kk_matrix, double *genotype_vector, double *phenotype_vector);

void lbfgsb(int n, int m, double *x, double *l, double *u, int *nbd,
	    double *Fmin, int *fail,
	    void *ex, double factr, double pgtol,
	    int *fncount, int *grcount, int maxit, char *msg,
	    int trace, int nREPORT, int Matrix_Size, double *kk_matrix, double *genotype_vector, double *phenotype_vector);
...
double loglike(int Para_Num, int Individual_Num, double *para, double *kk_matrix, double *genotype_vector, double *phenotype_vector);
int Load_data(int Individual_Num, string Filename_Ka, string Filename_Kaa, string Filename_Pheno, double *kk_matrix, double *phenotype_vector);
int parse_cmd_line(int argc, char* argv[], string & Filename_Ka, string & Filename_Kaa, string & Filename_Pheno, string & Filename_PGA_Out, int & Individual_Num);
...
{
   // cout<<"Variance Component Analysis..."<<endl;
	//cout << "Armadillo Start, and Armadillo version: " << arma_version::as_string() << endl;

	int Individual_Num;

	string Filename_Ka, Filename_Kaa, Filename_Pheno, Filename_PGA_Out;
	double *kk_matrix, *phenotype_vector, *genotype_vector;

    if(1==parse_cmd_line(argc, argv, Filename_Ka, Filename_Kaa, Filename_Pheno, Filename_PGA_Out, Individual_Num))
		return 1;

	kk_matrix       = new double [Individual_Num*(Individual_Num+1)];
	phenotype_vector= new double [Individual_Num];
	genotype_vector = new double [Individual_Num];
	for (int i=0; i<Individual_Num; i++) genotype_vector[i] =1.0;
	if (1==Load_data(Individual_Num, Filename_Ka, Filename_Kaa, Filename_Pheno, kk_matrix, phenotype_vector))
    {
       delete []kk_matrix;
	   delete []phenotype_vector;
	   delete []genotype_vector;
       return 1;
    }
    else
	{
		/* Load Kinship Matrix successed!*/
		/* Prepare the input parameters for optimization*/
		double para[3] = {1.0,1.0,1.0};
		int n, m, fail, fncount, grcount, maxit, trace, nREPORT;
		double *x, Fmin, factr, pgtol;
	    void *ex;
		char msg[60];

		ofstream parm_out_File; // used to output the optimized parameters
	     // From the data input
		n=3;  //7
		x=para;
		double lower[3]={1e-8,1e-8,1e-8};
		double upper[3]={1e8,1e8,1e8};
		double ndeps[3]={0.001, 0.001, 0.001}; // from R
		int    nbd[3]  ={2,2,2};
		...
		fncount        =0;
		grcount        =0;
		OptStruct OS =new opt_struct;
		OS->lower =lower;
		OS->upper =upper;
		OS->usebounds= 1;
		OS->ndeps   =ndeps;
		OS->parscale=parscale;
		OS->fnscale =fnscale;

		ex = (void*)OS;
		/* Call the optimization subroutine to */
		lbfgsb( n,  m,  x, lower, upper, nbd, &Fmin, &fail, ex, factr, pgtol, &fncount, &grcount, maxit, msg, trace, nREPORT,Individual_Num, kk_matrix, genotype_vector, phenotype_vector);
		delete OS;

		// output
		parm_out_File.open(Filename_PGA_Out.c_str());
		if(parm_out_File.is_open())
		{
			char delim =',';
			parm_out_File<< "Aditive"<< delim <<"Additive-Additive" <<delim<<"Residue"<<endl;
			parm_out_File<<para[0]<<delim<<para[1]<<delim<<para[2]<<endl;
		}
		else
		{
			cout <<"Error open the Polygenic Analysis(param optimization) result file!"<<endl;
            return 1;
		}
		parm_out_File.close();
        delete []kk_matrix;
...
int parse_cmd_line(int argc, char* argv[], string & Filename_Ka, string & Filename_Kaa, string & Filename_Pheno, string & Filename_PGA_Out, int & Individual_Num)
{
	if((argc!=2)&&(argc <9))
	{
		cerr <<"Please specific the correct parameters for polygenic component analysis!" <<endl;
		return 1;
	}
	else
	{
	   for (int i=1; i<argc;i++)
       {
		   ...
		   if((arg=="-o")||(arg=="-O"))
	       {
              if(i+1<argc)
		      {
		         Filename_PGA_Out = argv[i+1];
		      }
		      else
		      {
			     cerr<<"Parse cmd_line fail, Need to clearly specific the full name of Polygenic Analysis result(Optimized Lamada) file!"<<endl;
			     return 1; // Parse cmd_line fail
		      }
		    }
		    ...
		    if((arg=="-u")||(arg=="-U"))
	        {
				 ...
				 cout << "-o or -O: The full name of the Polygenic Analysis result(Optimized Lamada) file" <<endl;
...
/* This function is used to load the 2D Kinship matrixes and 1D phenotype vector from the 2 K-matirx files  and the phenotype file*/
int Load_data(int Individual_Num, string Filename_Ka, string Filename_Kaa, string Filename_Pheno, double *kk_matrix, double *phenotype_vector)
{
	ifstream K_File, Phenotype_File;
	string sLine;
	long Num_Count =0;
	...
	{
		cout <<"Load Kinship Matrix failed!" <<endl;
		return 1;
	}
	...
	{
		cout <<"Phenotype Vector load failed!" <<endl;
		return 1;
	}
...
/* here, the loglike function is the object function for optimization*/
double loglike(int Para_Num, int Individual_Num, double *para, double *kk_matrix, double *genotype_vector, double *phenotype_vector)
{
//	cout <<"Optimization param" <<endl;
//	for (int i_cout=0; i_cout< Para_Num; i_cout++) cout << para[i_cout]<<", ";
//	cout<<endl;
    double ret;
	mat V(Individual_Num, Individual_Num);
	...
	for (int i=0;i< Individual_Num;i++)
	{
		...
		{
			double tem =0.0;
			for (int i_Para =0; i_Para<Para_Num-1 ; i_Para++) tem += para[i_Para]*kk_matrix[i_Para*Trian_Matrix_Shift+Local_Shift];
			if(i!=j)
			{
			   V(i,j) =tem;
			   V(j,i) =tem;
			...
			{
			   V(i,j) =para[Para_Num-1]+tem;
			}
			Local_Shift ++;
		}
	}

/*	cout <<"ka(0,0)="<< kk_matrix[0] << " kd(0,0)"<<kk_matrix[278*279/2]<<" kaa(0,0)"<<kk_matrix[2*278*279/2] << " kad(0,0)" <<kk_matrix[3*278*279/2]<<" kda(0,0)"<<kk_matrix[4*278*279/2] <<" kdd(0,0)"<<kk_matrix[5*278*279/2]<<endl;
	cout <<"ka(0,1)="<< kk_matrix[1] << " kd(0,1)"<<kk_matrix[278*279/2+1]<<" kaa(0,1)"<<kk_matrix[2*278*279/2+1] << " kad(0,1)" <<kk_matrix[3*278*279/2+1]<<" kda(0,1)"<<kk_matrix[4*278*279/2+1] <<" kdd(0,1)"<<kk_matrix[5*278*279/2+1]<<endl;

	cout <<"V(0,0)="<< V(0,0) << " V(0,1)"<< V(0,1)<<" V(1,0)"<<V(1,0) << " V(277,276)" <<V(277,276)<<" V(276,277)"<<V(276,277) <<" V(277,277)"<<V(277,277)<<endl;*/
	mat V_I =inv(V);
	mat X(Individual_Num, 1);
	mat Y(Individual_Num, 1);
	for (int i_x=0; i_x<Individual_Num; i_x++) X(i_x, 0) =genotype_vector[i_x];
	for (int i_y=0; i_y<Individual_Num; i_y++) Y(i_y, 0) =phenotype_vector[i_y];

	mat XX =trans(X) * V_I * X;
	mat XY =trans(X) * V_I * Y;

	mat B  =solve( XX, XY);
	mat R  =Y-X*B;
	mat Q  =trans(R) * V_I * R;

	double d1, d2, sign1, sign2, q;
	log_det(d1,sign1, V);
	log_det(d2,sign2, XX);
	q =det(Q);
//	cout <<"Optimization Result" <<endl;
//	cout << "d1="<< d1<<", d2="<<d2 <<", q=" <<q<<endl;
	ret = 0.5*(d1+d2+q);
	return ret;
}
...
double fminfn(int n, double *p, void *ex, int Matrix_Size, double *kk_matrix, double *genotype_vector, double *phenotype_vector)
{
    ...
    for (i = 0; i < n; i++)
	{
	  if (!FINITE(p[i]))
	  {
		 printf("Error: non-finite value supplied by optim");
		 exit(-1);
	  }
	  x[i] = p[i] * (OS->parscale[i]);
    }

	val = loglike(n, Matrix_Size, x, kk_matrix, genotype_vector, phenotype_vector);
    val = val/(OS->fnscale);
...
void fmingr(int n, double *p, double *df, void *ex, int Matrix_Size, double *kk_matrix, double *genotype_vector, double *phenotype_vector)
{
	...
	double *x=new double [n];
	for (i = 0; i < n; i++) x[i] = p[i] * (OS->parscale[i]);
	...
	for (i = 0; i < n; i++)
    {
		...
		x[i] = tmp * (OS->parscale[i]);
		val1 = loglike(n, Matrix_Size, x, kk_matrix, genotype_vector, phenotype_vector);
		...
		x[i] = tmp * (OS->parscale[i]);
		val2 = loglike(n, Matrix_Size, x, kk_matrix, genotype_vector, phenotype_vector);
		val2 = val2/(OS->fnscale);

		df[i] = (val1 - val2)/(epsused + eps);
		if(!FINITE(df[i]))
		{
		 printf("Error: non-finite finite-difference value [%d]", i+1);
		 exit(-1);
	    }
		x[i] = p[i] * (OS->parscale[i]);
	}
...
/* n:  number of parameters for optimization
** m:  lmm, default 5
** x:  a vector pointer for the parameters
** l:  a vector pointer for lower boundary of the parameters
** u:  a vector pointer for upper boundary of the parameters
** nbd:infinite status for the lower and upper boundary
** Fmin: the evaluation value of object function
** fail:
** ex : a OP structure valiable, working as bridge between the object function and its gradient function
*/
void lbfgsb(int n, int m, double *x, double *l, double *u, int *nbd,
	    double *Fmin, int *fail,
	    void *ex, double factr, double pgtol,
	    ...
	    int trace, int nREPORT)*/
{
	...
	*fncount = 1;
	*grcount = 0;
	*Fmin = fminfn(n, u, ex, Matrix_Size, kk_matrix, genotype_vector, phenotype_vector);
	...
	*fail = 0;
    ...
    if (nREPORT <= 0)
	{
		 printf("Error: REPORT must be > 0 (method = \"L-BFGS-B\")");
		 exit(-1);
	}
...
//  g = vect(n);
    ...
    wa = new double[2*m*n+4*n+11*m*m+8*m];
...
/*	printf("in lbfgsb - %s\n", task);*/
	if (strncmp(task, "FG", 2) == 0) {
	    f = fminfn(n, x, ex, Matrix_Size, kk_matrix, genotype_vector, phenotype_vector);
	    if (!FINITE(f))
		{
		   printf("Error: L-BFGS-B needs finite values of 'fn'");
		   exit(-1);
	    }
//		error(_("L-BFGS-B needs finite values of 'fn'"));
	    fmingr(n, x, g, ex, Matrix_Size, kk_matrix, genotype_vector, phenotype_vector);
...
/*     Compute the infinity norm of the (-) projected gradient. */
    ...
    if (iprint >= 1)
	printf("At iterate %5d  f= %12.5g  |proj g|= %12.5g\n",
		iter, *f, sbgnrm);
...
L222:
    if (iprint >= 99) printf("Iteration %5d\n", iter);
    iword = -1;
...
/* Computing MAX */
    d__1 = fabs(fold), d__2 = fabs(*f), d__1 = math_max(d__1, d__2);
    ddum = math_max(d__1, 1.);
    if (fold - *f <= tol * ddum) {
/*					  terminate the algorithm. */
...
/*		     this variable is always fixed */
	...
	if (!*cnstnd) printf("This problem is unconstrained.\n");
    }
    if (iprint > 0)
	printf("At X0, %d variables are exactly at the bounds\n", nbdd);
 ...
 */
    ...
    if (*sbgnrm <= 0.) {
	if (iprint >= 0) printf("Subgnorm = 0.  GCP = X.\n");
...
/*		    is a zero vector, return with the initial xcp as GCP. */
	if (iprint > 100) {
	    printf("Cauchy X =  ");
	    for (int i = 1; i <= n; i++) printf("%g ", xcp[i]);
	    printf("\n");
	}
	return;
    }
...
/*     Initialize derivative f2. */
    ...
    tsum = 0.;
    *nint = 1;
    if (iprint >= 99) printf("There are %d  breakpoints\n", nbreak);
...
/*	 reset the corresponding component of d to zero. */
    ...
    if (iprint >= 100) printf("Variable  %d  is fixed.\n", ibp);
    if (nleft == 0 && nbreak == n) {
...
L888:
	...
	printf("Piece    %3i f1, f2 at start point %11.4e %11.4e\n",
		*nint,f1,f2);
	printf("Distance to the stationary point =  %11.4e\n", dtm);
    }
...
*/
    ...
    *ileave = n + 1;
	...
	for (int i = 1; i <= *nfree; ++i) {
		...
		indx2[*ileave] = k;
		if (iprint >= 100)
		    printf("Variable %d leaves the set of free variables\n",
			    k);
	    }
	}
	for (int i = *nfree + 1; i <= n; ++i) {
		...
		indx2[*nenter] = k;
		if (iprint >= 100)
		    printf("Variable %d enters the set of free variables\n",
			    k);
	    }
         if (iprint >= 100)
	     printf("%d variables leave; %d variables enter\n",
		     n + 1 - *ileave, *nenter);
	}
    }
...
/*     Find the index set of free and active variables at the GCP. */
    *nfree = 0;
    ...
    if (iprint >= 99)
	printf("%d  variables are free at GCP on iteration %d\n",
		*nfree, *iter + 1);
    return;
} /* freev */
/* ======================= The end of freev ============================== */
...
/*     Set the minimum and maximum steps allowed for stp. */
    if (brackt) {
	stmin = math_min(stx,sty);
	stmax = math_max(stx,sty);
    } else {
...
/* Computing MAX */
	d__1 = fabs(theta), d__2 = fabs(*dx),
	    d__1 = math_max(d__1,d__2), d__2 = fabs(*dp);
	s = math_max(d__1,d__2);
/* Computing 2nd power */
	d__1 = theta / s;
...
/*	     taken, otherwise the secant step is taken. */
	    ...
	    d__1 = *stp + (*sty - *stp) * .66;
	    if (*stp > *stx) {
		stpf = math_min(d__1,stpf);
	    } else {
		stpf = math_max(d__1,stpf);
	    }
	} else {
...
/*	     taken, otherwise the secant step is taken. */
	    ...
	    stpf = math_min(*stpmax,stpf);
	    stpf = math_max(*stpmin,stpf);
	}
...
/* Computing MAX */
	    d__1 = fabs(theta), d__2 = fabs(*dy), d__1 = math_max(d__1,d__2), d__2 =
		    fabs(*dp);
	    s = math_max(d__1,d__2);
/* Computing 2nd power */
	    d__1 = theta / s;
...
{
    if (iprint >=  0) {
	printf("N = %d, M = %d machine precision = %g\n", n, m, epsmch);
	if (iprint >= 100){
	    pvector("L =", l, n);
	    pvector("X0 =",x, n);
	    pvector("U =", u, n);
...
{
    if (iprint >=  99) {
	printf("LINE SEARCH %d times; norm of step = %g\n", iback, xstep);
	if (iprint > 100) {
	    pvector("X =", x, n);
	    pvector("G =", g, n);
	}
		   ...
		   int k)
{
	...
	if (iprint >= 1) printf("F = %g\n", *f);
    }
    if (iprint >= 0) {
	switch(info) {
	case -1: printf("Matrix in 1st Cholesky factorization in formk is not Pos. Def."); break;
	case -2: printf("Matrix in 2st Cholesky factorization in formk is not Pos. Def."); break;
	case -3: printf("Matrix in the Cholesky factorization in formt is not Pos. Def."); break;
	case -4: printf("Derivative >= 0, backtracking line search impossible."); break;
	case -5: printf("l(%d) > u(%d).  No feasible solution", k, k); break;
	case -6: printf("Input nbd(%d) is invalid", k); break;
	case -7: printf("Warning:  more than 10 function and gradient evaluations\n   in the last line search\n"); break;
	case -8: printf("The triangular system is singular."); break;
